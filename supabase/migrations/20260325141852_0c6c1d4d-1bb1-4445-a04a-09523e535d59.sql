...
CREATE POLICY "Users can delete own conversations" ON public.tutor_conversations FOR DELETE TO authenticated USING (auth.uid() = user_id);

CREATE POLICY "Users can view own messages" ON public.tutor_messages FOR SELECT TO authenticated USING (
  EXISTS (SELECT 1 FROM public.tutor_conversations c WHERE c.id = conversation_id AND c.user_id = auth.uid())
);
CREATE POLICY "Users can insert own messages" ON public.tutor_messages FOR INSERT TO authenticated WITH CHECK (
  EXISTS (SELECT 1 FROM public.tutor_conversations c WHERE c.id = conversation_id AND c.user_id = auth.uid())
);
